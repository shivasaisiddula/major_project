...
# Before balancing
plt.subplot(1, 2, 1)
plt.hist(df_copy_fea[47], bins=2, color=['blue'], alpha=0.7)
...
# After balancing
plt.subplot(1, 2, 2)
plt.hist(df_copy_fea.iloc[v][47], bins=2, color=['orange'], alpha=0.7)
plt.xticks([0, 1], ["Non-Seizure (0)", "Seizure (1)"])
...
# Sample feature names (replace with actual feature names)
feature_names = [str(i) for i in range(1, 48)]  # Assuming 47 features

fig, axes = plt.subplots(1, 2, figsize=(18, 6))

# Box plot before normalization
axes[0].boxplot(np.random.rand(100, 47) * 100000)  # Replace with actual data
axes[0].set_title("Feature Distribution Before Normalization")
axes[0].set_xlabel("Features")
axes[0].set_ylabel("Raw Values")
axes[0].set_xticks(range(1, len(feature_names) + 1))
axes[0].set_xticklabels(feature_names, rotation=90)  # Rotate labels for readability

# Box plot after normalization
axes[1].boxplot(np.random.rand(100, 47))  # Replace with actual normalized data
axes[1].set_title("Feature Distribution After Normalization")
axes[1].set_xlabel("Features")
axes[1].set_ylabel("Normalized Values")
axes[1].set_xticks(range(1, len(feature_names) + 1, 2))  # Show every 2nd label
axes[1].set_xticklabels(feature_names[::2], rotation=90)
